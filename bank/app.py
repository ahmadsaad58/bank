...
# Store Users by their unique username
USERS: Dict[str, User] = {}
# Store BankAccounts by their username
...
def update_account(user_name: str, account_name: str) -> str:
    ...
    try:
        if "account_type" in data:
            accounts[account_to_update].account_type = AccountType[data["account_type"].upper()]
        if "currency" in data:
            accounts[account_to_update].currency = data["currency"]
        if "balance" in data:
            accounts[account_to_update].balance = float(data["balance"])
        return (
            jsonify({"message": "Account updated successfully.", "accounts": accounts[account_to_update].serialize_dict()}),
            200,
        )
    except KeyError:
        return jsonify({"error": "Invalid account_type"}), 400
...
@app.route("/api/v1/accounts/<string:user_name>/<string:account_name>", methods=["DELETE"])
